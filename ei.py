...
from utils import scores, set_seed, random_integers, sample, retrieve_X_y, append_modality, metric_threshold_dataframes, \
    create_base_summary, safe_predict_proba, dummy_cv
...
class EnsembleIntegration:
    ...
    def predict(self, X_dictionary, meta_model_key):
        ...
        for i in range(len(self.modality_names)):
            ...
            assert X.shape[
                       1] == n_features, f"{X.shape[1]} features were given for {modality_name} modality, but {n_features} were used during training."
    ...
    def train_model_fold_sample(self, X, y, model_params, fold_params, sample_state, model_building=False):
        ...
        if model_building:
            ...
            results_dict = {
                ...
                "sample id": sample_id,
                "pickled model": pickle.dumps(model)
            ...
            for model_name in self.base_predictors.keys():
                for sample_id in range(self.n_samples):
                    model_predictions = list(d["y_pred"] for d in list_of_dicts if
                                             d["fold id"] == fold_id and d["model name"] == model_name and d[
                                                 "sample id"] == sample_id)
